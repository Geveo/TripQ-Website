...
const  AccountTransactions = () => {
    ...
    const currency = process.env.REACT_APP_CRYPTO_CURRENCY;
    ...
    const loadTransactions = () => {
        if (accountAddress) {
            init().then(res => {
                getTrustlines(accountAddress, process.env.REACT_APP_CRYPTO_CURRENCY, process.env.REACT_APP_CURRENCY_ISSUER).then(res => {
                    if (res && res.length > 0) {
    ...
    const pay = async () => {
        const result = await showPayQRWindow(
            ...
            "6",
            DestinationTags.RESERVATION_PAYMENT,
            process.env.REACT_APP_CRYPTO_CURRENCY,
        ...
        <>
            <Container style={{minHeight: "85vh"}}>
                ...
                <Row className={`mt-5`}>
                    ...
                    <Col lg={5}>
                        <Card className={`text-center p-5 balance-card`} style={{position: 'relative'}}>
                            <div style={{position: 'absolute', left: 10, top: 5}} className={`card-label`}> XAH
                                balance
                            </div>
                            <Card.Body style={{marginTop: '33px'}}>
                                            ...
                                            <td>
                                                <h5 className='mb-0'>
                                                    {tx.Account === accountAddress ?
                                                        <Badge bg="danger">SENT <FontAwesomeIcon icon={faArrowUp}
                                                                                                 style={{transform: 'rotate(45deg)'}}/>
                                                        </Badge> : <Badge bg="success">RECEIVED <FontAwesomeIcon
                                                            icon={faArrowUp} style={{transform: 'rotate(225deg)'}}/></Badge>}
                                                </h5>
                                            </td>
                                            <td>
                                                <div className='cell-texts'
                                                     style={{color: 'rgb(44 44 118)'}}>{formatAndGetDate(tx.date)}</div>
                                            </td>
                                            <td>
                                                <div className='cell-texts'
                                                     style={{color: 'rgb(44 44 118)'}}>{formatAndGetTime(tx.date)}</div>
                                            </td>
                                            <td>
                                                <div className='cell-texts'
                                                     style={{color: 'rgb(44 44 118)'}}>{tx.Account === accountAddress ? tx.Destination : tx.Account}</div>
                                            </td>
                                            <td>
                                                <div
                                                    className={`cell-texts ${tx.Account === accountAddress ? 'text-red' : 'text-green'} `}>{formatAmount(tx.Amount)}</div>
                                            </td>
                                        </tr>
                                    )
                                }
